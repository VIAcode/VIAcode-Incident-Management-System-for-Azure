                        ...
                        try {
                            ...
                            $jobs += Start-Job -ScriptBlock {
                                ...
                                while ($true) {
                                    $tries++
                                    try {                                        
                                        New-AzResourceGroupDeployment -Name "viacode_consulting-1089577.viacode-itsm-z-preview-$date" -ResourceGroupName $resourceGroup -TemplateFile "$dir\template.json" -functionSubscription $functionSubscription -functionResourceGroup $functionResourceGroup -functionName $functionName -location $location -AzContext $context
                                        break
                                    } catch {
                                        if($tries -ge 30){
                                            Throw
